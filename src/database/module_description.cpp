...
const std::map<std::string, boost::shared_ptr<Module_port>>&
Module_description::get_input_ports() const
{
	std::map<std::string, boost::shared_ptr<Module_port>> input_ports;
	for(const auto& [name, port] : m_ports)
	{
		if(port->get_type() == IN)
		{
...
void Module_description::printModule() const
{
	...
	for(const auto& [name, net] : m_nets)
	{
		net->printNet();
		std::cout<<std::endl;
	}

	std::cout << std::endl<<"~~~Instances~~~"<<std::endl;
