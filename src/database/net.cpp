...
void Net::printNet() const
{
	std::cout << "Name: " << m_name; 

	if(m_source_port != nullptr) 
	{
		std::cout << " Source Port: ";
		m_source_port->printPort();
		std::cout << std::endl;
	}

	if(!m_destination_ports.empty())
	{ 
		std::cout << "~~~Destination Ports~~~"<<std::endl;
		for(const auto& el : m_destination_ports) 
		{
			el->printPort();
			std::cout<<std::endl;
		}
