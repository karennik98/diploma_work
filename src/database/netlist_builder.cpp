...
void Netlist_builder::add_input_port( const std::string& info, const std::string& comment )
{
	...
	if(name.find(",") != std::string::npos) 
	{
		...
		size_t pos = 0;
		std::string token;
		while ((pos = name.find(delimater)) != std::string::npos)
		{
			token = name.substr(0, pos);
			eat_whitespaces(token);
			current_module->add_port(boost::shared_ptr<Module_port>(new Module_port(token, IN, current_module.get())));
			name.erase(0, pos + delimater.length());
		}	
	}
	eat_whitespaces(name);
	current_module->add_port(  boost::shared_ptr<Module_port>( new Module_port(name, IN, current_module.get())));
}

void Netlist_builder::eat_whitespaces(std::string& str)
{
	for(auto& el : str) 
	{
		if(el == ' ') 
...
void Netlist_builder::add_output_port( const std::string& info, const std::string& comment )
{
	...
	if(name.find(",") != std::string::npos) 
	{
		...
		while ((pos = name.find(delimater)) != std::string::npos)
		{
			token = name.substr(0, pos);
			eat_whitespaces(token);
			current_module->add_port(boost::shared_ptr<Module_port>(new Module_port(token, OUT, current_module.get())));
			name.erase(0, pos + delimater.length());
		}	
	}
...
void Netlist_builder::add_wire( const std::string& info, const std::string& comment )
{
	...
	if(name.find(",") != std::string::npos) 
	{
		...
		while ((pos = name.find(delimater)) != std::string::npos)
		{
			token = name.substr(0, pos);
			eat_whitespaces(token);
			current_module->add_net(boost::shared_ptr<Net>(new Net(token)));
			name.erase(0, pos + delimater.length());
		}	
	}
...
void Netlist_builder::add_module_instance( const std::string& info, const std::string& comment )
{
	...
	instance_name = rest.substr(0, rest.find("("));
	eat_whitespaces(instance_name);
	...
	if(dot_position = rest.find("."), dot_position != std::string::npos) 
	{
		while (true)
		{
